...
class HostConfigCollector:
    ...
    def _is_config_complete(self, config: Dict[str, Any]) -> bool:
        ...
        try:
            ...
            # 检查python字段是否有效
            if "python_version" not in config["python"] or config["python"]["python_version"] == "N/A":
                log.debug(f"配置缺少或无效的python_version")
                return False

            # 检查libraries是否为空或全是"Not installed"
            if not config["libraries"] or all(v == "Not installed" for v in config["libraries"].values()):
...
class JoplinConfigManager:
    ...
    def _merge_configs(self, parsed_config: Dict[str, Any], local_config: Dict[str, Any]) -> Dict[str, Any]:
        """合并解析的配置和本地配置"""
        ...
        if "system" in local_config:
            ...
            if "timestamp" not in merged["system"] or merged["system"]["timestamp"] == "N/A":
                merged["system"]["timestamp"] = local_config["system"].get("timestamp", "N/A")
            ...
            if "system" in local_config["system"]:
                for key in [
                    ...
                    if key not in merged["system"]["system"] or merged["system"]["system"][key] == "N/A":
        ...
        if "libraries" in local_config and local_config["libraries"]:
            if not merged["libraries"] or all(v == "Not installed" for v in merged["libraries"].values()):
                ...
                for lib, version in local_config["libraries"].items():
                    if lib not in merged["libraries"] or merged["libraries"][lib] == "Not installed":
                        merged["libraries"][lib] = version

        # 合并project字段
        if "project" in local_config:
            if "project_path" not in merged["project"] or merged["project"]["project_path"] == "N/A":
                merged["project"]["project_path"] = local_config["project"].get("project_path", "N/A")

            if "config_files" not in merged["project"] or not merged["project"]["config_files"]:
            ...
            for device_id, config_data in configs_dict.items():
                ...
                if self._is_config_complete(config_data):
                    ...
                    if device_id in local_configs:
                        log.info(f"设备 {collector.device_name} 的配置不完整，使用本地配置补充")
                        # 合并配置：以解析的配置为基础，用本地配置补充缺失字段
                        merged_config = self._merge_configs(config_data, local_configs[device_id])
    ...
    def generate_markdown_table(self, config_collectors: Dict[str, HostConfigCollector]) -> str:
        ...
        if not config_collectors:
            return "# 主机配置对比表\n\n暂无配置信息\n"
        # 获取所有设备ID并按设备名称排序
        device_ids = sorted(config_collectors.keys(), key=lambda x: config_collectors[x].device_name)
        md_lines = ["# 主机配置对比表\n"]
        # 1. 系统信息
        md_lines.append("\n## 1. 系统信息\n")
