...
# 银行长号码前缀（含 106 开头的服务号），
# 用于 `number.startswith(prefix)` 匹配识别发卡行
_DEFAULT_BANK_NUMBER_PREFIXES = {
    # 招商银行
    "95555": "招商银行",
    # 交通银行
    "95559": "交通银行", "10691779888": "交通银行",
    # 建设银行
    "95533": "建设银行", "106980095533": "建设银行",
    # 农业银行
    "95599": "农业银行", "1069095599": "农业银行",
    # 广发银行
    "95508": "广发银行", "106980095508": "广发银行",
    "106980095558": "广发银行", "1065902095558": "广发银行",
    # 其他银行
    "95588": "工商银行", "95595": "光大银行",
    "95568": "民生银行", "95528": "浦发银行",
    "95558": "中信银行", "95511": "平安银行",
    "95566": "中国银行", "95577": "华夏银行",
    "95561": "兴业银行", "95580": "邮储银行",
}
...
def get_bank_number_prefixes() -> dict:
    """获取银行号码前缀映射（含长号码），用于 number.startswith() 匹配。"""
    _ensure_loaded()
    return _cache.get("bank_number_prefixes", dict(_DEFAULT_BANK_NUMBER_PREFIXES))
