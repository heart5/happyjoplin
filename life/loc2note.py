...
with pathmagic.context():
    ...
    from etc.getid import getdevicename, getdeviceid
...
def register_new_device(new_id):
    """注册新设备"""
    if (current_ids_str := getcfpoptionvalue("hjdevices", 'DEVICES', 'ids')) is not None:
...
def parse_location_txt(fl):
    ...
    try:
        ...
        df = pd.read_csv(
            fl, sep='\t', header=None,
            parse_dates=[0], date_format='%Y-%m-%d %H:%M:%S',
            on_bad_lines='skip',
            names=['time', 'latitude', 'longitude', 'altitude', 
                   'accuracy', 'bearing', 'speed', 'unknown1', 
                   'unknown2', 'provider']
        )
...
def update_note_metadata(note_id, df, resource_id, is_new_device=False):
    ...
    # 更新附件链接
    content = re.sub(r"\[\S+\]\(:/\w+\)", f"[下载数据文件{jpapi.get_resource(resource_id).title}](:/{resource_id})", content)
...
def upload_to_joplin(file_path, device_id, period, save_dir):
    ...
    if existing_notes:
        ...
        note_body = f"""
## 位置数据元信息
- 时间范围: {period.start_time} 至 {period.end_time}
- 包含设备: {device_id}
...
def update_location_note(device_id, notebook_guid):
    ...
    try:
        ...
        parent_id = searchnotebook("位置信息数据仓")  # 自动创建新笔记本的逻辑已在searchnotebook中实现
