...
def all2df(name, wcdatapath):
    ...
    with lite.connect(dbname) as conn:
        ...
        log.critical(f"从数据库{dbfilename}中读取数据time字段为【文本】的数据共有{df_tmp.shape[0]}条，\
              最大值为：{df_tmp['time'].max()}，最小值为：{df_tmp['time'].min()}")
        db_all_df['time'] = db_all_df['time'].apply(lambda x: int(arrow.get(x, tzinfo='local').timestamp()) if type(x) != int else x)
    ...
    if items_all_num != dfdone.shape[0]:
        ...
        log.critical(f"从数据库{dbfilename}中读取数据并和文本中记录合并后，\
...
def spdf2liked(spdf):
    ...
    dupindexshortlist = dup_mul_df[dup_mul_df['content'].apply(lambda x: x.endswith("just liked your ranking"))].index.values
...
def wcliked2note():
    ...
    log.info(f'健康运动笔记【更新时间：{arrow.get(wc_sp_liked_items_num, tzinfo="local")}-》{noteupdatetimewithzone}】。')
    ...
    for i in range(len(dr) - 1):
        ...
        if dfp.shape[0] != 0:
            ...
            if not os.path.exists(fna):
                ...
                setcfpoptionvalue('happyjpwcitems', fn, 'itemsnumfromtxt',
                                  f"{dfp.shape[0]}")
            else:
...
def getnotelist(name, wcpath, notebookguid):
    ...
    if (numatlocal := getcfpoptionvalue('happyjpwcitems', "common", f"{name}_num_at_local")) is None:
        ...
        log.info(f"首次运行getnotelist函数，统计微信账户《{name}》的本地资源文件数量({numatlocal_actual})存入本地ini变量中")
